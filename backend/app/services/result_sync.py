...
class ResultSyncService:
    ...
    def _update_table_status(self, db, user_id: str, dataset_name: str, table_name: str, 
        ...
        try:
            ...
            log_error(f"Failed to update table status", e)
            return {
                "status": "error",
    ...
    def sync_results(
        ...
        try:
            ...
            if total_count == 0:
                ...
                return {"status": "success", "message": "No rows to sync", "completion_percentage": 0}
            ...
            while True:
                ...
                try:
                    ...
                    consecutive_failures = 0
                    last_progress_time = time.time()
                    log_info(f"Processed batch with {len(results)} results, completed {completed_count} rows so far")
                    
                except Exception as e:
                    log_error(f"Error syncing results for batch", e)
                ...
                if time.time() - last_progress_time > 300:
                    log_info(f"No progress for 5 minutes, completing sync process")
                    break
            ...
            log_info(f"Marked table {dataset_name}/{table_name} as {result['table_status']} ({completion_percentage:.1%} complete)")
            
            return result
            
        except Exception as e:
            log_error(f"Sync process terminated with error", e)
