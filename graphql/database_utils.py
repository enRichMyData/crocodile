...
class DatabaseManager:
    ...
    def create_dataset(self, dataset_name: str, tables: List[str]) -> Dict[str, Any]:
        ...
        dataset_id = self.dataset_collection.insert_one(
            {"dataset_name": dataset_name, "tables": tables}
        ).inserted_id
    ...
    def get_datasets(
        ...
        try:
            ...
            datasets = list(
                self.dataset_collection.find(query).sort("_id", 1).limit(page_size + 1)
            )
            ...
            for dataset in datasets[:page_size]:
                ...
                tables = list(
                    self.table_collection.find(
                        {"dataset_name": dataset_name}, {"table_name": 1, "_id": 0}
                    )
                )
                ...
                processed_datasets.append(
                    {"dataset_name": dataset_name, "tables": table_names, "status": current_status}
                )
            ...
            return {"datasets": processed_datasets, "next_cursor": next_cursor}

        except Exception as e:
            logging.error(f"Error getting datasets: {e}")
            return {"datasets": [], "next_cursor": None}
    ...
    def get_tables(
        self, dataset_name: str, cursor: Optional[str] = None, page_size: int = 5
    ) -> Dict[str, Any]:
    ...
    def get_table_results(
        self, dataset_name: str, table_name: str, cursor: Optional[str] = None, page_size: int = 5
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not metadata:
                ...
                metadata = self.dataset_collection.find_one(
                    {"dataset_name": dataset_name},
                    {"dataset_name": 1, "total_rows": 1, "start_time": 1, "_id": 0},
                )
                    ...
                    if not isinstance(metadata["classified_columns"], list):
                        ...
                        if isinstance(metadata["classified_columns"], str):
                            metadata["classified_columns"] = [
                                col.strip() for col in metadata["classified_columns"].split(",")
                            ]
    ...
    def get_table_results_with_annotations(
        ...
        try:
            ...
            # Get basic results
            results = list(
                self.input_data_collection.find(query).sort("_id", 1).limit(page_size + 1)
            )
            ...
            return {"results": return_results, "next_cursor": next_cursor}
        except Exception as e:
    ...
    def set_todo_status(self, dataset_name: str, table_name: str):
        ...
        self.input_data_collection.update_many(
            {"dataset_name": {"$ne": dataset_name}}, {"$set": {"status": "SKIPPED"}}
        )
        ...
        todo_count = self.input_data_collection.count_documents(
            {"dataset_name": dataset_name, "status": "TODO"}
        )
    ...
    def insert_dataset_status(self, dataset_name, status):
        ...
        self.status_collection.update_one(
            {"dataset_name": dataset_name}, {"$set": {"status": status}}, upsert=True
        )

    def check_and_update_dataset_status(self, dataset_name: str) -> str:
        """
        Check the actual processing status based on input_data
        records and update status accordingly.
        """
        ...
        total_count = self.input_data_collection.count_documents({"dataset_name": dataset_name})

        logging.info(
            f"Status counts - TODO: {todo_count}, DONE: {done_count}, Total: {total_count}"
        )
        ...
        self.status_collection.update_one(
            {"dataset_name": dataset_name},
            {
                "$set": {
                    ...
                    "stats": {"total": total_count, "done": done_count, "todo": todo_count},
                }
            },
            upsert=True,
        )
        ...
        self.dataset_collection.update_one(
            {"dataset_name": dataset_name},
            {"$set": {"status": new_status, "last_updated": datetime.datetime.utcnow()}},
            upsert=True,
        )
