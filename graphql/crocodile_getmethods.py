...
class Query:
    ...
    @strawberry.field
    def get_datasets(
        self, cursor: Optional[str] = None, page_size: int = 5, direction: str = "next"
    ) -> PaginatedDatasets:
        query = {}
        ...
        datasets = list(
            db.dataset_trace.find(query)
            .sort("_id", 1 if direction == "next" else -1)
            .limit(page_size)
        )
        ...
        has_next_page = (
            bool(db.dataset_trace.find_one({"_id": {"$gt": ObjectId(next_cursor)}}))
            if next_cursor
            else False
        )
    ...
    def get_tables_in_dataset(
        ...
        has_next_page = (
            bool(db.table_trace.find_one({"_id": {"$gt": ObjectId(next_cursor)}}))
            if next_cursor
            else False
        )
    ...
    def get_table_data(
        ...
        has_next_page = (
            bool(db.input_data.find_one({"_id": {"$gt": ObjectId(next_cursor)}}))
            if next_cursor
            else False
        )
